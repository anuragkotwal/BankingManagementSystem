    ...
    for name in all_acc:
        ...
        if uname == name:
            ...
            if passcode == paswrd:
                ...
                # #Buttons
                Button(dash_screen, text="Personal Details", font=('Courier New', 13), width=20,
                       command=lambda: p.personal_deatil(uname, root), bg='#FFFFFF').place(x=340, y=100)
                Button(dash_screen, text="Deposit", font=('Courier New', 13), width=20,
                       command=lambda: d.deposit(uname, root), bg='#FFFFFF').place(x=340, y=140)
                Button(dash_screen, text="Withdraw", font=('Courier New', 13), width=20,
                       command=lambda: w.withdraw(uname, root), bg='#FFFFFF').place(x=340, y=180)
                Button(dash_screen, text="Logout", command=lambda: login_acc(root), font=('Courier New', 13),
